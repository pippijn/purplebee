...
namespace uiops
{
  ...
  {
    return server->call<void*> (OPS "request_input", title, primary, secondary, default_value, multiline, masked, hint, ok_text, ok_cb, cancel_text, cancel_cb, account, who, conv, user_data);
  }
  ...
  {
    return server->call<void*> (OPS "request_choice", title, primary, secondary, default_value, ok_text, ok_cb, cancel_text, cancel_cb, account, who, conv, user_data, choices);
  }
  ...
  {
    return server->call<void*> (OPS "request_action", title, primary, secondary, default_action, account, who, conv, user_data, action_count, actions);
  }
  ...
  {
    return server->call<void*> (OPS "request_fields", title, primary, secondary, fields, ok_text, ok_cb, cancel_text, cancel_cb, account, who, conv, user_data);
  }
  ...
  {
    return server->call<void*> (OPS "request_file", title, filename, savedialog, ok_cb, cancel_cb, account, who, conv, user_data);
  }

  void
  request::close_request (PurpleRequestType type, void* ui_handle)
  {
    return server->call<void> (OPS "close_request", type, ui_handle);
  }
  ...
  {
    return server->call<void*> (OPS "request_folder", title, dirname, ok_cb, cancel_cb, account, who, conv, user_data);
  }
